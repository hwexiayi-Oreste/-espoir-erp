...
async function initDB() {
  if (isPG) {
    // PostgreSQL — un statement à la fois
    const tables = [
      ...
      `CREATE TABLE IF NOT EXISTS espoir_chantiers (
        id SERIAL PRIMARY KEY, reference TEXT UNIQUE,
        nom TEXT, client_id INTEGER REFERENCES espoir_clients(id),
    ...
    for (const t of tables) await db.execAsync(t);
  } else {
    await db.execAsync(`
      CREATE TABLE IF NOT EXISTS espoir_utilisateurs (
...
async function seedData() {
  ...
  // Utilisateurs - un par un avec runAsync
  const users = [
  ...
  for (const u of users) {
    await db.runAsync(
      'INSERT INTO espoir_utilisateurs (nom,prenom,email,mot_de_passe,role,telephone) VALUES (?,?,?,?,?,?)',
      u
    );
  }
  ...
  for (const d of devis) {
    await db.runAsync(
      'INSERT INTO espoir_devis (reference,client_id,service,montant_ht,tva,montant_ttc,statut,date_emission,date_validite,chef_projet,description,created_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)',
      d
    );
  }
  ...
  for (const c of chantiers) {
    await db.runAsync(
      'INSERT INTO espoir_chantiers (reference,nom,client_id,type_service,lieu,chef_chantier,date_debut,date_fin_prevue,avancement,budget,depense,statut,notes) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)',
      c
    );
  }
  ...
  for (const a of acts) {
    await db.runAsync(
      'INSERT INTO espoir_activite (user_id,action,detail) VALUES (?,?,?)',
      a
    );
  }
}
